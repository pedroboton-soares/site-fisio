...
**Segunda correção, encontrada na revisão da etapa 7:** o id `regiao` estava duplicado no HTML, usado tanto na seção "Onde atendo" quanto no campo do formulário de contato. Isso é inválido em HTML e quebrava a associação do `<label>` com o campo certo. Corrigido renomeando o id e o `for` do label do campo do formulário para `regiao-contato` (mantendo o `name="regiao"` do campo, que é o que importa pro envio dos dados), e ajustando a referência correspondente no `script.js`.

## Etapa 7 - Revisão em outra conversa/ferramenta

Prompt usado, numa conversa nova, no Gemini (ferramenta diferente da que gerou o código, o ChatGPT):
```
...
O retorno marcou os 10 requisitos funcionais e os 7 não funcionais como cumpridos, cada um apontando a linha correspondente no código. Fora dos requisitos, encontrou dois pontos: o id `regiao` duplicado no HTML (corrigido, ver etapa 6), e o fato de o formulário só mostrar um `alert()` no envio, sem integração real, o que é esperado já que o projeto não tem backend.
